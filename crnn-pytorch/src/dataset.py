...
class CustomDataset(Dataset):
    with open(os.path.join('data/', 'chars/ch_tra_char.txt'), encoding='utf-8') as fr:
        chars = fr.read().splitlines()
    ...
    def _load_from_raw_files(self, root_dir, mode):
        ...
        files_path = os.path.join(root_dir, paths_file)

        print(files_path)

        for file in os.listdir(files_path):
            if os.path.isfile(os.path.join(files_path, file)):
                paths.append(os.path.join(files_path, file))               
                texts.append(file.split('_')[0].replace(' ', ''))
